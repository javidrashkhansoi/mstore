  ...
  if (minus && plus && input && output) {
    [minus, plus].forEach(button => {
      button.addEventListener("click", () => {
        changeValue(button === minus ? "minus" : "plus");
      });
    });
    ...
    function changeValue(action) {
      ...
      value = +value;
      input.value = action === "plus" ? ++value : --value;
...
if (searchButton) {
  const addingClass = "search-show";

  searchButton.addEventListener("click", () => {
    classList.toggle(addingClass);
  });
  ...
  document.addEventListener("click", event => {
    ...
    if (!target.closest(".header")) classList.remove(addingClass);
  });
...
/** @type {HTMLDivElement} */
const radiosBlock = document.querySelector(".product-radios");
/** @type {HTMLButtonElement} */
const radiosButton = radiosBlock?.querySelector(".radio-button");
/** @type {HTMLDivElement} */
...
if (radiosButton) {
  /** @type {HTMLDivElement} */
  const radiosBlockInner = radiosBlock?.querySelector(".product-radios__inner");
  /** @type {NodeListOf<HTMLInputElement} */
  const radios = radiosBlock?.querySelectorAll("input");
  /** @type {HTMLSpanElement} */
  const buttonText = radiosButton.querySelector(".radio-button__text");

  if (radiosBlockInner) {
    radiosButton.addEventListener("click", () => {
      radiosButton.classList.toggle("show");
    });
    ...
    document.addEventListener("click", event => {
      ...
      if (!target.closest(".product-radios")) {
        radiosButton.classList.remove("show");
      }
    });
  ...
  if (radios.length && buttonText) {
    radios.forEach(radio => {
      radio.addEventListener("change", () => {
        buttonText.innerText = getCheckedRadioInnerText();
      });
    });

    function getCheckedRadioInnerText() {
      /** @type {[HTMLInputElement]} */
      const [checkedRadio] = [...radios].filter(/** @param {HTMLInputElement} radio */ radio => radio.checked);
      /** @type {HTMLSpanElement} */
      const textElement = checkedRadio.parentElement.querySelector(".product-radios__text");
...
descriptionButton?.addEventListener("click", () => {
  descriptionBlock.classList.add("show");
...
class Dialogs {
  ...
  #dialogCloseEvent(event) {
    const { target } = event;
    const [dialog] = this.#dialogsArray.filter(dialogObject => dialogObject.$dialog === target);
    const { $button, $dialog } = dialog;
...
;// CONCATENATED MODULE: ./src/js/libraries/nouislider/ranges/price-slider.js
...
if (uiSlider) {
  const { dataset } = uiSlider;
  /** @type {HTMLInputElement} */
  const fromInput = document.querySelector("input[data-from]");
  /** @type {HTMLInputElement} */
  const toInput = document.querySelector("input[data-to]");
  /** @type {HTMLOutputElement} */
  const fromOutput = document.querySelector(".fieldset-numbers__value--from");
  /** @type {HTMLOutputElement} */
  const toOutput = document.querySelector(".fieldset-numbers__value--to");

  let { min, max, step, start, end, margin } = dataset;

  min = !!min ? +min.trim() : 0;
  max = !!max ? +max.trim() : 1500;
  step = !!step ? +step.trim() : 100;
  start = !!start ? +start.trim() : 500;
  end = !!end ? +end.trim() : 1000;
  margin = !!margin ? +margin.trim() : 100;
  ...
  if (fromInput && toInput && fromOutput && toOutput) {
    const inputs = [fromInput, toInput];
    const outputs = [fromOutput, toOutput];
    ...
    uiSlider.noUiSlider.on("update", (values, handle) => {
      ...
      outputs[handle].innerText = values[handle];
    });

    fromInput.addEventListener("change", () => {
      uiSlider.noUiSlider.set([+fromInput.value, null]);
    });
    ...
    toInput.addEventListener("change", () => {
      uiSlider.noUiSlider.set([null, +toInput.value]);
    });
...
;// CONCATENATED MODULE: ./src/js/libraries/choices/selects/category-sort.js


/** @type {HTMLSelectElement} */
const category_sort_select = document.querySelector(".catalog-sort select");

if (category_sort_select) {
  const choices = new scripts_choices(category_sort_select, {
    allowHTML: false,
    searchEnabled: false,
    position: "bottom",
    itemSelectText: "",
